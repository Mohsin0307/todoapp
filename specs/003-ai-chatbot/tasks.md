...
- [X] T013 Create backend/mcp_tools/ directory structure with __init__.py
- [X] T014 Create backend/mcp_tools/schemas.py with JSON schemas for all 5 tools
- [X] T015 Implement add_task tool in backend/mcp_tools/task_tools.py with validation
- [X] T016 [P] Implement get_tasks tool in backend/mcp_tools/task_tools.py with status filtering
- [X] T017 [P] Implement update_task_status tool in backend/mcp_tools/task_tools.py
- [X] T018 [P] Implement delete_task tool in backend/mcp_tools/task_tools.py
- [X] T019 [P] Implement get_task_statistics tool in backend/mcp_tools/task_tools.py
- [X] T020 Create MCP tool registration mechanism in backend/mcp_tools/__init__.py
- [X] T021 Add tool registration to backend/main.py startup event handler
- [X] T022 Verify MCP tools registered successfully in backend logs (should show "Registered 5 MCP tools")

### OpenAI Agents SDK Integration

- [X] T023 Create backend/services/agent_service.py with Agent initialization pattern (per-request)
- [X] T024 Implement system prompt for task management agent in backend/services/agent_service.py
- [X] T025 Implement conversation history loading with 50-message windowing in backend/services/conversation_service.py
- [X] T026 Implement error handling with retry logic using tenacity in backend/services/agent_service.py
- [X] T027 Create conversation management service in backend/services/conversation_service.py for create/update/get operations

### Chat API Endpoint

- [X] T028 Create backend/routers/chat.py with POST /api/{user_id}/chat endpoint
- [X] T029 Implement JWT authentication middleware for chat endpoint in backend/routers/chat.py
- [X] T030 Implement request validation (message length, conversation_id) in backend/routers/chat.py
- [X] T031 Implement conversation creation or continuation logic in backend/routers/chat.py
- [X] T032 Integrate agent_service with chat endpoint for message processing in backend/routers/chat.py
...
- Within a story: Most tasks are sequential (verification → prompt update → testing)
...
2. Add User Story 1 → Test independently → (Can demo task creation via chat)
3. Add User Story 2 → Test independently → (Can demo full CRUD read via chat) ✅ **MVP READY**
4. Add Frontend → Test UI → Deploy/Demo
5. Add User Story 3 → Test independently → (Task updates via chat)
6. Add User Story 4 → Test independently → (Task deletion via chat)
7. Add User Story 5 → Test independently → (Analytics via chat) ✅ **FULL FEATURE COMPLETE**
...
- US1: Send "Add a task to buy groceries" → Verify task in database
- US2: Send "What's pending?" → Verify task list returned
- US3: Send "Mark buy groceries as done" → Verify status updated in database
- US4: Send "Delete buy groceries" → Verify task removed from database
- US5: Send "How am I doing?" → Verify statistics calculation correct

**Suggested Implementation Order**: Setup → Foundational → US1 → US2 → Frontend → US3 → US4 → US5 → Polish
